...
int main(int argc, const char *argv[]) {
	...
	if (!rf69.init()) {
		...
		// Defaults after init are 434.0MHz, +13dBm, no encryption,
		// GFSK=250kbps, Fdev=250kHz
		...
		while (!force_exit) {
			...
			if (bcm2835_gpio_eds(RF_IRQ_PIN)) {
				...
				// printf("Packet Received, Rising event detect for pin
				// GPIO%d\n", RF_IRQ_PIN);
#endif
				...
				if (rf69.available()) {
					...
					if (rf69.recv(buf, &len)) {
						printf("Packet[%02d] #%d => #%d %ddB: ", len, from, to,
							   rssi);
						printbuffer(buf, len);
					} else {
